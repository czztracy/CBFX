...
<template>
  <!-- 分组重命名遮罩 -->
  <el-dialog :visible.sync="shadedata.dialogFormVisible" :title="'“'+shadedata.name + '” ' + $t('templateManager.groupRename')" class="tree-shade">
...
<script>
import { templateClassRename } from '@/api/modle'
export default {
  name: 'NewGroup',
  props: {
    shadedata: {
  ...
  data() {
    return {
      form: {
        templateClassId: '', // 模板分类id,必填
        templateClassName: '', // 模板分类名称,必填
        ptemplateClassId: '', // 父级模板分类id,必填
        templateCode: '', // templateCode,必填
        templateLevel: '' // 模板分类级别,必填（级别为1和2的不能修改）
      },
      rules: {
        templateClassName: [
  ...
  methods: {
    ...
    confirmName(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // 父级模板分类id,必填
          this.form.ptemplateClassId = this.shadedata.targetData.data.pid
          // 模板分类id 必填
          this.form.templateClassId = this.shadedata.targetData.data.id
          // 模板分类级别,必填（级别为1和2的不能修改）
          this.form.templateLevel = this.shadedata.targetData.data.level
          // 模板分类代码 必填
          this.form.templateCode = this.shadedata.targetData.data.code
          var father = this.shadedata.targetData.node.parent.data
          templateClassRename(this.form).then(res => {
            // console.log(res)
            if (res.success) {
              this.$message({
                message: '重命名成功',
